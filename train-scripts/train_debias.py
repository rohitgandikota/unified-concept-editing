...
def get_ratios(ldm_stable, prev_ratio, ratio_diff, max_ratio_gap, concepts, classes, num_samples=10, num_loops=3):
    ...
    for idx, concept in enumerate(concepts):
        ...
        probs_full = []
        test_prompts = [f'picture of {class_}' for class_ in classes[idx]]
        with torch.no_grad():
            for seed in seeds:
    #             if i == num_loops:
    #                 break
                ...
                for cnt, t in enumerate(zip(old_texts, new_texts)):
                    ...
                    with torch.no_grad():
                        for layer in projection_matrices:
                            ...
                            target = o_embs + (weights[cnt][0]*o_embs.norm())*u1 + (weights[cnt][1]*o_embs.norm())*u2
...
if __name__ == '__main__':
    ...
    print_text += f"_sd_{args.base.replace('.','_')}" 
