...
#erasure_counts = [1, 5, 10, 50, 100, 500,1000, 1500]
#erasure_counts = [1,5]
#erasure_counts = [10,50]
#erasure_counts = [100,500]
erasure_counts = [100]
...
#erasure_counts = [1, 5, 10, 25, 50, 100, 500, 700]
#erasure_counts = [1,5]
#erasure_counts = [10,25]
#erasure_counts = [50,100]
#erasure_counts = [500, 700]
preserve_idxs = np.arange(len(artists))
random.shuffle(preserve_idxs)
preserve_idxs = preserve_idxs[:1]
preserve_artists = []
non_preserve_artists = []
for i, artist in enumerate(artists):
    if i in preserve_idxs:
        preserve_artists.append(artist)
    else:
        non_preserve_artists.append(artist)
for count in erasure_counts:
    erase_idxs = np.arange(len(non_preserve_artists))
    random.shuffle(erase_idxs)
    erase_idxs = erase_idxs[:count]
    erase_artists = []
    
    for i, artist in enumerate(non_preserve_artists):
        if i in erase_idxs:
            erase_artists.append(artist)

    print(f'Erasing {len(erase_artists)} artists | Preserving {len(preserve_artists)}')
    with open(f'data/fixed_erase{count}.txt', 'w') as fp:
    ...
    ######### EDIT THE MODEL
    edit_model(old_text_ = preserve_artists, new_text_='', retain_text_= erase_artists, lamb=0.5)
#     edit_model(old_text_=erase_artists, new_text_='', retain_text_= None, lamb=0.5)
    torch.save(ldm_stable.unet.state_dict(), f'models/diffusers-erasing-1-with-{count}-preservation.pt')
