...
if __name__=='__main__':
    ...
    model, preprocess = dreamsim(pretrained=True, device=f"cuda:{args.device}")
    if True:
        ...
        for index, row in df_prompts.iterrows():
            ...
            for file in files:
                print(file)
                try:
                    img1 = preprocess(Image.open(os.path.join(args.original_path,file))).to(f"cuda:{args.device}")
                    img2 = preprocess(Image.open(os.path.join(args.edited_path,file))).to(f"cuda:{args.device}")
                    l = model(img1, img2)
                    print(f'Dreamsim score: {l.item()}')
        ...
        if args.save_path is not None:
            ...
            df_prompts.to_csv(os.path.join(args.save_path, f'{basename}_dreamloss.csv'))
